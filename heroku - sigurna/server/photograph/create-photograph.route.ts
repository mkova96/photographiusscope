...
module.exports = async function createPhotograph(req:Request, res:Response) {
    ...
    if ((!Number.isInteger(req.body.year) || !Number.isInteger(req.body.price)) || 
        (req.body.name.length >50 || req.body.name.length <1) || (req.body.year <1800 || req.body.year > 2020) || (req.body.price <1)){        winston.log({level:'error',message:'ADMIN: Error in create-photograph'},date)
